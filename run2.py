"""i am messing about with code from run.py after having not looked at it in a long time"""
...
import time
from helpers import normalise,mean,median,std,get_roi,get_contour_for_label,getContours,getWidth,getWidths,getHeights
...
while(cap.isOpened()):
  ...
  #print(threshold_image.dtype)
  threshold_image = threshold_image.astype('float64')
  #print(threshold_image.max())
  #print(threshold_image.dtype)
  means = np.array([mean(label,threshold_image,segments_slic) for label in all_labels])
  medians = np.array([median(label,threshold_image,segments_slic) for label in all_labels])
  #print(means.max())
  #print(means.min())
  ...
  segment_statistics = np.concatenate((means,medians,0.5*widths,0.5*heights),axis=1)
  #print(means.shape)

  #segment_statistics = means

  #am i normalising correctly should i not be nomralising by number of pixels in each segment
